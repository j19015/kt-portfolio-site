...
React Three Fiber の粒子表現の中を、猫の後ろについて歩いていく1枚のランディングページ。
...
3Dは暗い床の上を**2匹の猫が歩き、足跡が増えていく**。スクロールに合わせてカメラが前へ進み、
猫はその少し先を歩き続ける。床には**経歴1件につき1つ**、窓から差した明かりが落ちていて、
`data/career.json` の件数が変われば明かりの数も自動で追従する。
...
- **猫は顔を描いていない**。目や鼻を置いた瞬間にキャラクターになるので、光るのは輪郭だけにして
  「暗がりに浮かぶシルエット」に留めている。猫だと分かるのは造形ではなく歩き方の側に持たせていて、
  接地順（左後→左前→右後→右前）と接地率0.78という実際の猫の歩容をそのまま解いている
- **接地中の足はワールド座標に固定している**。スクロール速度がどう変わっても足が地面を滑らず、
  足跡は必ず足が着いた場所に残る
